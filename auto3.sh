...
do
	...
	crtsh -d "$domain" --recursive | sort -u > crt.txt || echo -e "${RED}crt.sh timed out or failed${NC}"
...
httpx -t 100 -sc -title -cl -probe -l subs.txt | grep -v "FAILED" > live.txt
...
nuclei -l new.txt -et ssl -nh -nmhe -nss -es info,low -headless -o vulns.txt
...
dirsearch -l juice_subs.txt --config ~/.config/dirsearch/config.ini -t 75 --skip-on-status 429 > "dirsearch.txt"
